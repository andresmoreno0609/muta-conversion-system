package com.sistem.conversion.template;

import com.sistem.conversion.exception.UnitConversionException;
 ...
 *   <li><b>runMetrics</b> – log success and optionally record metrics.</li>
 ...
 * application. By enforcing a common structure (input → validation → core logic → metrics/logging → side
 * effects) and a single place for logging and exception mapping, it:
 * <ul>
 ...
 *   <li>Centralizes cross-cutting concerns: logging, metrics and HTTP error mapping are implemented once
...
public abstract class UseCaseAdvance<RQ, RS> {
    ...
    /**
     * Runs the use case: preConditions → core → runMetrics → postConditions.
     ...
     * @throws .UnitConversionException when conversion rules fail (handled by ConversionExceptionHandler).
     */
    ...
    /**
     * Called after a successful core execution. Logs success and can be extended for metrics.
     * Subclasses should not override; override {@link #postConditions(Object)} for custom behaviour.
     *
     * @param response the result returned by {@link #core(Object)}.
     */
    protected final void runMetrics(RS response) {
        log.info("UseCase [{}] completed successfully. Response: {}",
                this.getClass().getSimpleName(),
                response != null ? response.toString() : "null");
    }

    /**
     * Optional hook run after success (after runMetrics). Override for audit, notifications, or other side effects.
     *
     ...
     * @throws UnitConversionException when rethrowing conversion errors (422).
     */
    protected void handleException(Exception ex) {
        ...
        if (ex instanceof UnitConversionException uce) {
            throw uce;
        }
